...
export const TenderProvider = ({ children }: TenderProviderProps) => {
  ...
  const fetchTenders = async () => {
    try {
      ...
      const response = await fetch(`https://api.smarttender.rio.software/api/recent-tenders?user_id=${userId}`);
  ...
  const fetchAllTenders = async (page: number) => {
    try {
      ...
      // Use the new API endpoint with user_id
      const userId = '123'; // Replace with actual user ID management
      const response = await fetch(`https://api.smarttender.rio.software/api/all-tenders?user_id=${userId}`);
