...
const UploadArea = () => {
  ...
  const handleFileUpload = async (file: File) => {
    ...
    try {
      ...
      const response = await fetch('http://192.168.2.71:8000/upload-tender', {
        method: 'POST',
        body: formData,
      });
  ...
  const handleAnalyze = async (tenderId: number) => {
    ...
    try {
      ...
      const analyzeUrl = `http://192.168.2.71:8000/analyze-tender?tender_id=${tenderId}&user_id=123&org_name=${encodeURIComponent(orgName.trim())}`;
  ...
  const fetchSectionStatus = async (tenderId: number) => {
    ...
    try {
      const statusUrl = `http://192.168.2.71:8000/section-status?tender_id=${tenderId}&user_id=123&org_name=${encodeURIComponent(orgName.trim())}`;
